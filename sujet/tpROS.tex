...
\begin{itemize}
 ...
 En cas d'erreur on pourra utiliser le script \texttt{dh\_code.py} qui genere le code correspondant a un tableau de Denavit-Hartenberg, voir Annexe \ref{sec:DH}.
 ...
 Notez qu'il suffit de definir les points $P_1$ et $P_2$, le code inversant la consigne en fonction de la frequence etant deja ecrit en debut de boucle.
 Ce code fait qu'a tout instant, $P_0$ est l'ancienne consigne (le point d'ou on vient) et $P_d$ est la nouvelle (le point ou on va).
...
\begin{itemize}
 ...
 Le script decrit en Annexe \ref{sec:DH} genere le code du jacobien en fonction du tableau des parametres.
...
\section{Generation de code avec parametre de Denavit-Hartenberg}
\label{sec:DH}

Le script \texttt{dh\_code.py} permet de generer le code C++ du modele geometrique direct et du jacobien a partir du tableau des parametres.\\
Un exemple est donne dans le package \texttt{tps\_robot/urdf/dh\_example.yml}. Pour le lancer :
\begin{itemize}
 \item \texttt{roscd tps\_robot/urdf}
 \item \texttt{rosrun tps\_robot dh\_code.py dh\_example.yml}
\end{itemize}
N'hesitez pas a creer votre propre fichier YAML (extension \texttt{.yml}) dans votre package afin de ne pas avoir a faire les calculs a la main. 
